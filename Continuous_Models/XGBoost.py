...
def perform_rfecv(X, y):
    ...
    rfecv = RFECV(estimator=base_model, step=1, cv=5, n_jobs=-1, min_features_to_select=5)
    rfecv.fit(X, y)
    return rfecv
...
def train_model(best_params, X_train, y_train, selector_type):
    model = XGBRegressor(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'}, random_state=42)
...
def evaluate_model(model, selector, X_train, y_train, X_test, y_test, preprocessed_feature_names, model_name):
    ...
    plt.tight_layout()
    plt.savefig(f'actual_vs_predicted-{model_name}.-XGB.png', bbox_inches='tight')
    plt.show()
...
def plot_feature_importance(importance_df, categorical_cols, categorical_features, model_name):
    ...
    plt.ylabel('Features')
    plt.title("Feature Importance's - XGB")
    plt.tight_layout()
    plt.savefig(f'feature_importances-{model_name}-XGB.png', bbox_inches='tight')
    plt.show()
...
print("\nModel Complexities:")
print(f"SelectFromModel - Number of trees: {model_sfm.n_estimators}")
print(f"RFE - Number of trees: {model_rfe.n_estimators}")
print(f"Full Model - Number of trees: {model_full.n_estimators}")
...
def plot_shap_values(model, X, feature_names, categorical_cols, categorical_features, model_name):
    ...
    plt.title(f"SHAP Feature Importance - XGB")
    ...
    plt.title(f"SHAP Summary Plot - XGB")
