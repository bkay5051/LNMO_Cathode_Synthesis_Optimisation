...
def train_final_model(best_params, X_train, y_train):
    XGB_best_model = XGBClassifier(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'},
                                           random_state=42)
    selector = SelectFromModel(XGB_best_model, max_features=best_params['n_features_to_select'], threshold=-np.inf)
    X_train_selected = selector.fit_transform(X_train, y_train)
    XGB_final_model = XGBClassifier(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'},
                                            random_state=42)
    XGB_final_model.fit(X_train_selected, y_train)
    return XGB_final_model, selector, XGB_best_model, X_train_selected
...
def plot_feature_importance(importance_df, categorical_cols, categorical_features):
    ...
    plt.tight_layout()
    plt.savefig('feature_importances-XGB-class.png', bbox_inches='tight')
    plt.show()
...
def plot_optimization_history(study):
    ...
    plt.title('Optimization History of the XGBoost Model')
    plt.savefig('ophistory-XGB-class.png', bbox_inches='tight')
    plt.show()
...
def plot_param_importances(study):
    ...
    plt.title('Hyperparameter Importances of the XGBoost Model')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('param_importances-XGB-class.png', bbox_inches='tight')
    plt.show()
...
final_model_XGB, selector, best_model_XGB, X_train_selected = train_final_model(best_params, X_train, y_train)

# Evaluate final model
logging.info("Evaluating model...")
importance_df, X_test_selected = evaluate_model(final_model_XGB, selector, X_test, y_test, preprocessed_feature_names)
...
y_pred = final_model_XGB.predict(X_test_selected)
test_accuracy = accuracy_score(y_test, y_pred)
print(classification_report(y_test, y_pred))
print(confusion_matrix(y_test, y_pred))
print("Test accuracy:", test_accuracy)

y_pred = final_model_XGB.predict(X_train_selected)
