...
def train_final_model(best_params, X_train, y_train):
    RF_best_model = RandomForestClassifier(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'},
                                           random_state=42)
    selector = SelectFromModel(RF_best_model, max_features=best_params['n_features_to_select'], threshold=-np.inf)
    X_train_selected = selector.fit_transform(X_train, y_train)
    RF_final_model = RandomForestClassifier(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'},
                                            random_state=42)
    RF_final_model.fit(X_train_selected, y_train)
    return RF_final_model, selector, RF_best_model, X_train_selected
...
def plot_feature_importance(importance_df, categorical_cols, categorical_features):
    ...
    plt.tight_layout()
    plt.savefig('feature_importances-RF-class.png', bbox_inches='tight')
    plt.show()
...
def plot_optimization_history(study):
    ...
    plt.title('Optimization History of the Random Forest Model')
    plt.savefig('ophistory-RF-class.png', bbox_inches='tight')
    plt.show()
...
def plot_param_importances(study):
    ...
    plt.title('Hyperparameter Importances of the Random Forest Model')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('param_importances-RF-class.png', bbox_inches='tight')
    plt.show()
...
final_model_RF, selector, best_model_RF, X_train_selected = train_final_model(best_params, X_train, y_train)

# Evaluate final model
logging.info("Evaluating model...")
importance_df, X_test_selected = evaluate_model(final_model_RF, selector, X_test, y_test, preprocessed_feature_names)
...
# Evaluate the final model on the test data RF
y_pred = final_model_RF.predict(X_test_selected)
test_accuracy = accuracy_score(y_test, y_pred)
print(classification_report(y_test, y_pred))
print(confusion_matrix(y_test, y_pred))
print("Test accuracy:", test_accuracy)

y_pred = final_model_RF.predict(X_train_selected)
