...
def data_prep():
    ...
    # Calculate the median discharge capacity
    median_capacity = df['Discharge Capacity [mAh/g]'].median()

    # Create binary target variable
    y = (df['Discharge Capacity [mAh/g]'] > median_capacity).astype(int)

    # Separate input features
    X =  df.drop(columns=['Discharge Capacity [mAh/g]', 'Coulombic efficiency%', 'Capacity Retention (%)', 'Capacity Retention (Cycles)', 'Rate Capability (mAh/g)', 'Rate Capability (C rate)'])
