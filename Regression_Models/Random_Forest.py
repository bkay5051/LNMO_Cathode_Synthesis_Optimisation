...
def optimize_model(objective, X_train, y_train, rfecv):
    ...
    try:
        study.optimize(objective_with_data, n_trials=200, n_jobs=-1)
...
def train_model(best_params, X_train, y_train, selector_type):
    model = RandomForestRegressor(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'}, random_state=42)
...
def plot_feature_importance(model, selector, preprocessed_feature_names, categorical_cols, categorical_features, model_name):
    if selector is not None:
        selected_indices = selector.get_support(indices=True)
        selected_preprocessed_features = [preprocessed_feature_names[i] for i in selected_indices]
        importances = model.feature_importances_
    ...
    aggregated_importances = importance_df.copy()
...
plot_feature_importance(model_sfm, selector_sfm, preprocessed_feature_names, categorical_cols, categorical_features, "SelectFromModel")
plot_feature_importance(model_rfe, selector_rfe, preprocessed_feature_names, categorical_cols, categorical_features, "RFE")
plot_feature_importance(model_full, None, preprocessed_feature_names, categorical_cols, categorical_features, "Full")
...
print("\nModel Complexities:")
print(f"SelectFromModel - Number of non-zero coefficients: {np.sum(model_sfm.feature_importances_ != 0)}")
print(f"RFE - Number of non-zero coefficients: {np.sum(model_rfe.feature_importances_ != 0)}")
print(f"Full Model - Number of non-zero coefficients: {np.sum(model_full.feature_importances_ != 0)}")
...
def safe_percentage_improvement(new_value, base_value, epsilon=1e-5):
    if abs(base_value) < epsilon:
        if abs(new_value - base_value) < epsilon:
            return 0.0  # No significant difference
        else:
...
def plot_shap_values(model, X, feature_names, categorical_cols, categorical_features, model_name):
    ...
    plt.title(f"SHAP Feature Importance - RF")
    ...
    plt.title(f"SHAP Summary Plot - RF")
