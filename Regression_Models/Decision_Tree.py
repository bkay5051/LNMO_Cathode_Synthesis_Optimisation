...
def train_model(best_params, X_train, y_train, selector_type):
    model = DecisionTreeRegressor(**{k: v for k, v in best_params.items() if k != 'n_features_to_select'}, random_state=42)
...
def plot_feature_importance(model, selector, preprocessed_feature_names, categorical_cols, categorical_features, model_name):
    ...
    plt.ylabel('Features')
    plt.title("Feature Importance's - DT")
    plt.tight_layout()
...
def plot_shap_values(model, X, feature_names, categorical_cols, categorical_features, model_name):
    ...
    plt.title(f"SHAP Feature Importance - DT")
    ...
    plt.title(f"SHAP Summary Plot - DT")
